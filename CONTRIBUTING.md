...
Installation, persistent selection, and one-off execution are separate actions. `install` never
changes a selection. The ordinary dispatcher resolves `AVM_ARGOCD_VERSION`, the nearest project
pin, then the personal default. Only `exec` takes an explicit per-invocation selector.
